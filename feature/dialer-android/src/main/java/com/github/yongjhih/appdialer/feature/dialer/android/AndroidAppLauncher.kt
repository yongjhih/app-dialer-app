...
class AndroidAppLauncher(
    ...
    override fun launchApp(app: AppModel) {
        try {
            Logger.i(TAG) { "Launching app: ${app.label} (${app.packageName}/${app.className})" }
            val launchIntent = context.packageManager.getLaunchIntentForPackage(app.packageName)
            if (launchIntent != null) {
                context.startActivity(launchIntent)
                onDismiss()
            } else {
                Logger.w(TAG) { "Launch intent not found for package: ${app.packageName}" }
                Toast.makeText(context, context.getString(R.string.cannot_launch_app, app.label), Toast.LENGTH_SHORT).show()
            }
        } catch (e: Exception) {
            Logger.e(TAG, e) { "Error launching app: ${app.label}" }
    ...
    override fun openAppDetails(app: AppModel) {
        try {
            Logger.i(TAG) { "Opening app details: ${app.label} (${app.packageName})" }
            val intent = Intent(
            ...
            Logger.e(TAG, e) { "Error opening app details for: ${app.packageName}" }
    ...
    override fun openSystemAppSettings() {
        try {
            Logger.i(TAG) { "Opening AppDialer system app settings" }
            val intent = Intent(
                Settings.ACTION_APPLICATION_DETAILS_SETTINGS,
            ...
            Logger.e(TAG, e) { "Error opening system app settings" }
